...
pub struct Mesh {
    ...
    pub skipped: usize,
    /// Точка, относительно которой пишутся координаты.
    ///
    /// Чертежи часто стоят в геодезических координатах — у проверочного файла
    /// это 750 000 по X. В 32-битном числе на таком удалении шаг между
    /// соседними значениями около шести сантиметров, и модель на выходе
    /// зернится. Поэтому первая же встреченная точка становится началом
    /// отсчёта, а вычитание делается здесь, пока счёт идёт в двойной точности.
...
impl Mesh {
    ...
    fn push_tri(&mut self, a: V3, b: V3, c: V3, flip: bool) {
        ...
        let o = *self.origin.get_or_insert(a);
