...
Собирается **в собственную базу**, наружу ничего не уходит: на VPS мало места
под ещё один контейнер, а данные о пользователях лучше не отдавать сторонним
сервисам. Персональных данных нет — ни IP, ни cookie, только анонимный
идентификатор вкладки, который исчезает вместе с ней.

Шаги: зашли на сайт → начали регистрацию → зарегистрировались → создали проект
→ импортировали местность → разместили объект. Считаются **уникальные люди**,
а не события.

Сводка — в кабинете, раздел «Воронка». Доступ по списку email:
```bash
# в ~/vovplan/.env на сервере
ANALYTICS_ADMIN_EMAILS=ваш@email.ru
```
Пусто — сводку не видит никто. Сбор событий при этом идёт всё равно.
