...
export default function ActivityPanel({ projectId }: { projectId: string }) {
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto text-slate-200 h-full overflow-y-auto">
      ...
      <p className="text-sm text-slate-400 mb-5">Кто сейчас в проекте и что менялось — в реальном времени.</p>

      {/* Сейчас онлайн */}
      <div className="glass p-4 mb-4">
        <h3 className="hud-title mb-3">Сейчас онлайн · {online.length}</h3>
        {online.length === 0 ? (
          <p className="text-sm text-slate-500">Пока никого — вы единственный посетитель.</p>
        ) : (
          <div className="flex flex-wrap gap-2">
            {online.map((p) => (
              <span key={p.userId} className="flex items-center gap-2 bg-white/5 rounded-full pl-1.5 pr-3 py-1">
                <span className="w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-bold text-white" style={{ background: p.color }}>
      ...
      <div className="glass p-4">
        ...
        {isLoading ? (
          <p className="text-sm text-slate-500 py-2">Загрузка...</p>
        ) : events.length === 0 ? (
          <p className="text-sm text-slate-500 py-2">Изменений пока нет. Разместите объект или нарисуйте сеть — событие появится здесь.</p>
        ) : (
              ...
              return (
                <li key={e.id} className="flex items-start gap-3 py-2 border-t border-white/5 first:border-0">
                  ...
                  <div className="flex-1 min-w-0 text-sm">
                    <span className="font-medium text-slate-200">{e.actorName}</span>
                    <span className="text-slate-400"> {meta.verb}</span>
                    {e.targetName && <span className="text-slate-300"> «{e.targetName}»</span>}
                  </div>
                  <span className="text-[11px] text-slate-500 flex-shrink-0 whitespace-nowrap">{timeAgo(e.createdAt)}</span>
                </li>
              );
            })}
          </ul>
